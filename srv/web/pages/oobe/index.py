        ...
        modules.append(
            {
                ...
                "author": manifest.get("developer") or manifest.get("author") or "",
            }
        )
...
if(serverTime&&serverDate){const d=new Date(serverTime.dataset.iso);if(!Number.isNaN(d.getTime())){serverTime.textContent=new Intl.DateTimeFormat(undefined,{hour:'numeric',minute:'2-digit'}).format(d);serverDate.textContent=new Intl.DateTimeFormat(undefined,{weekday:'long',month:'short',day:'numeric',year:'numeric'}).format(d);}}
    ...
    return f"""<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Setup - {h(settings["product_name"])}</title>{f'<link rel="icon" href="{h(settings["favicon"])}" type="image/x-icon">' if settings.get("favicon") else ""}<style>{OOBE_STYLE}</style><link rel="stylesheet" href="/assets/theme.css"></head><body><div class="page">{logo_html(settings)}<main class="wrap"><section class="card">{alerts}{content}</section></main></div>{script}</body></html>"""
