...
function configure_safe_name($value) {
    return preg_match('/^[A-Za-z0-9_-]+$/', (string)$value) === 1;
}

function configure_xml_text($root, $tag, $default = '') {
    $node = $root->{$tag} ?? null;
    return $node !== null ? trim((string)$node) : $default;
}

function configure_module_info($moduleDir, $module) {
    ...
    $info['name'] = configure_xml_text($root, 'name', $module) ?: $module;
    $info['description'] = configure_xml_text($root, 'desp') ?: configure_xml_text($root, 'description');
    $info['input_type'] = configure_xml_text($root, 'type', 'Output') ?: 'Output';
    return $info;
}

$module = $_GET['module'] ?? '';
$modulesRoot = realpath(__DIR__ . '/../../endpoint-modules');
$moduleDir = $modulesRoot && configure_safe_name($module) ? realpath($modulesRoot . DIRECTORY_SEPARATOR . $module) : false;
if (!$modulesRoot || !$moduleDir || strpos($moduleDir, $modulesRoot) !== 0 || !is_file($moduleDir . DIRECTORY_SEPARATOR . 'endpoint-forms' . DIRECTORY_SEPARATOR . 'forms.php')) {
    http_response_code(404);
    echo "Endpoint module not found";
    exit;
}
$moduleInfo = configure_module_info($moduleDir, $module);
$frameSrc = '/admin/endpoint-form-frame.php?' . http_build_query(['module' => $module]);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>Add <?= htmlspecialchars($moduleInfo['name']) ?> Endpoint - <?= htmlspecialchars($product_name) ?></title>
...
.form-frame { width:100%; min-height:760px; border:0; border-radius:6px; background:#FFF; box-sizing:border-box; display:block; }
...
<div id="content" onclick="closeSidebarOnContentClick()">
    <div class="header-actions">
        <div>
            <h1><?= htmlspecialchars($moduleInfo['name']) ?> Endpoint</h1>
            <?php if ($moduleInfo['description'] !== ''): ?>
                <p class="muted"><?= htmlspecialchars($moduleInfo['description']) ?></p>
            <?php endif; ?>
        </div>
        <a class="back-link" href="/admin/new-endpoint.php"><i class="fa-solid fa-arrow-left"></i> Modules</a>
    </div>
    <div class="frame-shell">
        <iframe class="form-frame" sandbox="allow-forms allow-same-origin allow-scripts allow-top-navigation" src="<?= htmlspecialchars($frameSrc) ?>" title="<?= htmlspecialchars($moduleInfo['name']) ?> endpoint form"></iframe>
    </div>
</div>
<script>
