...
$moduleDir = $modulesRoot && safe_name($module) ? realpath($modulesRoot . DIRECTORY_SEPARATOR . $module) : false;
if (!$modulesRoot || !$moduleDir || strpos($moduleDir, $modulesRoot) !== 0 || !is_file($moduleDir . DIRECTORY_SEPARATOR . 'settings.php')) {
    http_response_code(404);
    echo "Module settings not found";
    exit;
}
$moduleInfo = module_info($moduleDir, $module);
$frameSrc = '/admin/endpoint-module-settings-frame.php?' . http_build_query(['module' => $module]);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title><?= h($moduleInfo['name']) ?> Settings - <?= h($product_name) ?></title>
...
<div id="content" onclick="closeSidebarOnContentClick()">
    <div class="header-actions">
        <div>
            <h1><?= h($moduleInfo['name']) ?> Settings</h1>
            <?php if ($moduleInfo['description'] !== ''): ?>
                <p class="muted"><?= h($moduleInfo['description']) ?></p>
            <?php endif; ?>
        </div>
        <a class="back-link" href="/admin/endpoint-module-settings.php"><i class="fa-solid fa-arrow-left"></i> Modules</a>
    </div>
    <iframe class="settings-frame" sandbox="allow-forms allow-same-origin allow-scripts" src="<?= h($frameSrc) ?>" title="<?= h($moduleInfo['name']) ?> settings"></iframe>
</div>
<script>
